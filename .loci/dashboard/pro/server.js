#!/usr/bin/env node
/**
 * Loci Dashboard Pro — Node.js Local Server
 *
 ...
 *
 * Static files served from parent directory (../) for index-pro.html
 */
...
const PARENT_DIR = path.dirname(SCRIPT_DIR); // .loci/dashboard/
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/' || pathname === '/index.html') {
    // Serve index-pro.html from pro directory
    filePath = path.join(SCRIPT_DIR, 'index-pro.html');
  } else {
    // Try pro directory first, then parent directory
    const proPath = path.join(SCRIPT_DIR, pathname);
    const parentPath = path.join(PARENT_DIR, pathname);
  ...
  const resolvedParent = path.resolve(PARENT_DIR);
  if (!resolvedPath.startsWith(resolvedScript) && !resolvedPath.startsWith(resolvedParent)) {
...
server.listen(PORT, () => {
  console.log(`Loci Dashboard Pro`);
