...
const SYSTEM_PROMPT = [
  ...
  '     用返回的准确名字填 --people / --location；本次对话里已经跑过 names 就复用之前的结果，不要每条消息重跑。',
  '     例外——名单可能刚变了就重跑一次：用户说"我刚加了/改了某人某地"、用户坚持某人已存在但你的名单里没有、',
  '     或你已记不清之前的结果。绝不因为名单里没有就直接断言"这个人没存过"——先重跑一次再下结论。',
  '     绝不用 ls/grep/Read 去翻 people/ places/ 目录核对名字——names 一条命令就是全部答案。',
