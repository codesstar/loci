...
// mtime-keyed parse cache. /api/data re-reads and re-renders every markdown
// file in the brain on EVERY request (page load, every post-write refetch,
// every SSE-triggered refresh) — but between requests almost nothing changed.
// Keying on (mtimeMs, size) makes the whole rebuild cost proportional to what
// actually changed instead of to brain size. NOTE: cached objects are shared
// across requests — builders may only mutate them idempotently (reset-then-
// fill, like buildPeople's meta.connections), never accumulate.
const MD_CACHE_MAX = 5000;
const mdCache = new Map(); // filepath → { mtimeMs, size, value }
...
function readMdFile(filepath) {
  ...
  try {
    ...
    if (mdCache.size >= MD_CACHE_MAX) mdCache.clear(); // crude but bounded
    mdCache.set(filepath, { mtimeMs: stat.mtimeMs, size: stat.size, value });
    return value;
  } catch (e) {
    return null;
  }
...
// active.md is regenerated by every WRITER (saveTaskRecords here, the CLI's
// mutateTasks) — a GET must not write to disk. The once-a-day refresh below
// only exists so the view's stale/recently-done annotations roll over at
// midnight even on a day with no task writes.
let activeViewDay = '';
...
function buildTasks() {
  ...
  if (activeViewDay !== today) {
    activeViewDay = today;
    try { writeActiveTaskView(taskRecords); } catch { /* view refresh is best-effort */ }
  }
...
//
// Cached: buildProjects() runs on EVERY /api/data request, and each git
// subprocess costs 20-50ms fork+exec (worst case 2s on a cold disk) of
// event-loop freeze, times N projects. .git/logs/HEAD's mtime changes on
// every commit/checkout, so it's a precise invalidation key; the short TTL
// only exists to refresh the relative "%cr" timestamps ("2 days ago").
const gitLogCache = new Map(); // repoPath|limit → { headMtime, at, value }
const GIT_LOG_TTL_MS = 5 * 60 * 1000;

function readProjectGitLog(repoPath, limit = 150) {
  const cacheKey = repoPath + '|' + limit;
  let headMtime = 0;
  try { headMtime = fs.statSync(path.join(repoPath, '.git', 'logs', 'HEAD')).mtimeMs; } catch { /* not a repo / fresh repo */ }
  const hit = gitLogCache.get(cacheKey);
  if (hit && hit.headMtime === headMtime && Date.now() - hit.at < GIT_LOG_TTL_MS) return hit.value;
  const value = readProjectGitLogUncached(repoPath, limit);
  gitLogCache.set(cacheKey, { headMtime, at: Date.now(), value });
  return value;
}

function readProjectGitLogUncached(repoPath, limit) {
  try {
...
// Cached forever per repo: the connect date is the EARLIEST progress entry —
// new entries can only be later, so once known it never changes (and computing
// it re-reads every historical progress file).
const connectedDayCache = new Map(); // repoPath → 'YYYY-MM-DD'

function projectConnectedDay(repoPath) {
  const hit = connectedDayCache.get(repoPath);
  if (hit) return hit;
  try {
    let earliest = '';
    ...
    if (earliest) connectedDayCache.set(repoPath, earliest);
    return earliest;
  } catch { return ''; }
}
...
const sessionDigestCache = new Map(); // "path|date" → { mtimeMs, digest|null }

function rememberSessionDigest(key, mtimeMs, digest) {
  if (sessionDigestCache.size >= 2000) sessionDigestCache.clear();
  sessionDigestCache.set(key, { mtimeMs, digest });
}
...
function readSessionsForDay(dateStr) {
  ...
  for (const { fp, kind, mtimeMs } of capped) {
    // Transcript files run to tens of MB; on a busy day this loop used to
    // re-read and re-JSON.parse hundreds of MB per page view. A finished
    // session's file never changes — key the parsed digest on mtime.
    const ck = fp + '|' + dateStr;
    const cached = sessionDigestCache.get(ck);
    if (cached && cached.mtimeMs === mtimeMs) {
      if (cached.digest) {
        const d = cached.digest;
        out.push({ ...d, live: (nowMs - mtimeMs) < 900000 });
      }
      continue;
    }
    ...
    if (!startMs || (turns === 0 && !summary)) {
      // remember the negative too — an irrelevant file stays irrelevant
      rememberSessionDigest(ck, mtimeMs, null);
      continue;
    }
    ...
    const digest = {
      ...
      turns, compacted, file: fp,
    };
    rememberSessionDigest(ck, mtimeMs, digest);
    out.push({ ...digest, live: (nowMs - mtimeMs) < 900000 });
  }
...
// Cached: this walks the ENTIRE brain tree stat-ing every entry, on every
// /api/data request, to produce one cosmetic number. 60s of staleness on a
// stats counter is free; the walk on a big brain is not.
let totalFilesCache = null; // { at, count }

function countTotalFiles() {
  if (totalFilesCache && Date.now() - totalFilesCache.at < 60000) return totalFilesCache.count;
  let count = 0;
  function walk(dir) {
    let entries;
  ...
  walk(LOCI_ROOT);
  totalFilesCache = { at: Date.now(), count };
  return count;
}
...
function sendJson(res, data, statusCode = 200) {
  // Compact on purpose: /api/data is ~2MB — pretty-printing added 30-40%
  // dead weight to every response and none of the consumers are human.
  const json = JSON.stringify(data);
  // Same-origin app: no CORS headers issued — cross-origin reads are refused.
  const accept = String((res.req && res.req.headers['accept-encoding']) || '');
  if (json.length > 10240 && /\bgzip\b/.test(accept)) {
    // level 1: ~10ms for 2MB and still ~5x smaller — the right trade for a
    // single-threaded server whose remote clients are on slow links
    const gz = zlib.gzipSync(json, { level: 1 });
...
function serveStaticFile(res, filePath) {
  ...
  const compressible = /^(text\/|application\/(json|javascript|xml))/.test(mime);
  ...
  fs.stat(filePath, (statErr, stat) => {
    ...
    // mtime+size ETag: index.html is 1.4MB and rarely changes — without this,
    // every page open re-transfers it in full ("no-cache" allows revalidation
    // but there was nothing to revalidate against).
    ...
    fs.readFile(filePath, (err, data) => {
      ...
      if (compressible && data.length > 10240 && /\bgzip\b/.test(accept)) {
        headers['Content-Encoding'] = 'gzip';
        res.writeHead(200, headers);
        res.end(zlib.gzipSync(data, { level: 1 }));
        return;
      }
      res.writeHead(200, headers);
      res.end(data);
