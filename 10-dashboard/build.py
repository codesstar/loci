...
def build_inbox():
    ...
    inbox_items = scan_md_files(os.path.join(LOCI_ROOT, "00-inbox"))
    return {
...
def build_me():
    me_dir = os.path.join(LOCI_ROOT, "01-me")
...
def build_tasks():
    tasks_dir = os.path.join(LOCI_ROOT, "05-tasks")
...
def build_people():
    people_dir = os.path.join(LOCI_ROOT, "04-people")
    return {
...
def build_decisions():
    decisions_dir = os.path.join(LOCI_ROOT, "07-decisions")
...
def build_finance():
    finance_dir = os.path.join(LOCI_ROOT, "02-finance")
    return {"files": scan_md_files(finance_dir)}


def build_content():
    """Read content files and platform data from 06-content/."""
    content_dir = os.path.join(LOCI_ROOT, "06-content")
...
def build_learning():
    learning_dir = os.path.join(LOCI_ROOT, "06-content", "learning")
...
def count_total_files():
    ...
    for root, dirs, files in os.walk(LOCI_ROOT):
        dirs[:] = [d for d in dirs if not d.startswith(".") and d != "10-dashboard"]
        for f in files:
            if f.endswith(".md"):
                count += 1
    return count
