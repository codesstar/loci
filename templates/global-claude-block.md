...
- **Connecting a serious project (do ALL of these, don't skip any):** when the user is clearly committing to a project in its repo, on their go-ahead: (1) create `.loci/memory.md` with real substance (goal, current state, the triggering decision); (2) create `.loci/decisions/` and write the decision there; (3) **inject the Loci project block into BOTH the repo's `CLAUDE.md` and `AGENTS.md`, creating either file if absent** — so Claude Code (reads `CLAUDE.md`) and Codex (reads `AGENTS.md`) both auto-load this project's memory next session. The block is bracketed by `<!-- loci:project:start v1 -->` … `<!-- loci:project:end -->` and tells future sessions to read `.loci/memory.md` and record decisions in `.loci/decisions/`; if a block already exists, replace it in place. (4) append `.loci/` to the repo's `.gitignore` (create it if absent); (5) add a one-line index entry to `<brain-path>/projects/index.md`. Never copy the project's full memory into the brain.
