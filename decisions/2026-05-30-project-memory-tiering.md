...
brain holds a pointer (the index line in `me/projects.md`).

- **Brain side (`~/loci`)** holds only:
  - `me/projects.md` — one 2-4 line index entry per real project (what it is,
    current status, one-line essence). This is the single overview surface.
  - `.loci/links/<name>/` — symlink to the project repo + `to-hq.md` / `from-hq.md`
    for two-way sync. Projects are independent repos; their code/detail never moves
    into the brain.
...
- **What floats up to the brain (essence) — tag-first + AI fallback:**
  - Tag-first: anything tagged `[decision]` / `[insight]` / `[milestone]` /
    `[architecture]` auto-pushes to the brain via the project's `to-hq.md`.
    `[local]` / `[debug]` / `[wip]` stay in the project repo and the brain never sees them.
...
- [ ] Add a `people` routing rule and a `project` routing rule into `AGENTS.md` /
...
- [ ] Populate `me/projects.md` with real index entries for the user's active projects
      (currently empty template).
- [ ] Create a `.loci/memory.md` template for project repos capturing: goals / current
      state / next step, project-level decisions, progress log, related people.
- [ ] Clean up `.loci/links/registry.md` — remove the `client-acme` placeholder row.
