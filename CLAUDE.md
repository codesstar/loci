...
7. **Time = both places** — When a task mentions a specific time (e.g. "3点", "15:00", "morning meeting at 9"), write it to BOTH: (a) `tasks/daily/YYYY-MM-DD.md` as a checklist item formatted as `- [ ] 任务名 — HH:MM` (task name first, time after dash), AND (b) `tasks/calendar.json` as a calendar event with `startKey`/`endKey` in minutes from midnight (shows on Day Manifest timeline). Tasks without a time only go to the daily plan (no time suffix). If no end time is given, default to 1 hour duration
